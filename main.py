...
RSS_FEEDS = [
    ...
    {"name": "머니투데이",    "url": "https://news.mt.co.kr/mtview/rss/economy.xml"},
]

# ── 1. RSS에서 오늘 뉴스 수집 ─────────────────────────
def fetch_rss_news(max_per_feed=10):
    today_str  = now_kst.strftime("%Y-%m-%d")
    date_nums  = re.sub(r"\D", "", today_str)
    articles   = []
    ...
    for feed in RSS_FEEDS:
        try:
            ...
            for item in items:
                ...
                pub_nums = re.sub(r"\D", "", pub_date)

                if date_nums in pub_nums or pub_date == "":
                    articles.append({
                        "source": feed["name"],
                        "title":  title,
                        "desc":   desc,
            ...
            print(f"⚠️  {feed['name']} RSS 수집 실패: {e}")
...
def generate_card_html(articles):
    ...
    prompt = f"""
아래는 {TODAY_KR} 오늘 실제로 보도된 한국 경제·금융 뉴스 기사들이야.
  ...
  <div class="body">
    <div class="news-item">
      ...
      <div class="news-content">
        ...
        <div class="news-body">[본문 3~4문장]</div>
      </div>
    </div>
    <div class="news-item">
